...
await test("with attributes", async (t) => {
	await t.test("initial", async (t) => {
		...
		await t.test("should give a parent", () => {
			assert.equal(
				buildFileAs(dataPathUnit("canon.JPG"), File).getParentValue()
					.current.i_f_path_full.current,
	...
	await t.test("expected", async (t) => {
		...
		await t.test("should calculate a canonicalFilename", function () {
			...
			assert.equal(
				buildFileAs(f1, File).i_f_filename.expected,
				"2018-02-04"
			);
...
await test("with values", async function (t) {
		...
		async function (t) {
			await t.test("should read the data", async function (t) {
				await t.test("should extract extension", () => {
					...
					assert.equal(
						buildFileAs(f1, File).i_f_extension.initial,
						".txt"
					);

					const f2 = createEmptyUnitFile("a");
					assert.equal(
						buildFileAs(f2, File).i_f_extension.initial,
						""
					);
				});
				...
				await t.test("should extract filename", () => {
					{
						...
						getParentOf(f).reset();
						assert.equal(
							buildFileAs(f, File).i_f_filename.initial,
							"a.txt"
						);
					}
				...
				await t.test(
					...
					function () {
						const f1 = createEmptyUnitFile(
							"test_rename/brol/a.txt"
						);
			...
			await t.test("with fix File.i_f_path_full", async function (t) {
				...
				await t.test("from File.i_f_path_full", async () => {
					const filename = createEmptyUnitFile(
						"file-test-1/test-pathfull.txt"
					);
					const f = buildFileAs(filename, File);
	...
	await t.test(
		"File.i_00_f_exists and File.i_f_is_folder",
		async function (t) {
			await t.test("should read the data", async function (t) {
